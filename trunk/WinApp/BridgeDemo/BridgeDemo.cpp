// BridgeDemo.cpp : Defines the entry point for the application.
//
...
const double g_FixedTimeStep = 0.1;
BridgeNode::BridgeNodePtr g_pBridge(new BridgeNode);
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_BRIDGEDEMO, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, (LPCTSTR)IDC_BRIDGEDEMO);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, (LPCTSTR)IDC_BRIDGEDEMO);
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= (LPCTSTR)IDC_BRIDGEDEMO;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDM_EXIT:
			DestroyWindow(hWnd);
			break;
		//case ID_STUFF_USEPENALTY:
		//	OutputDebugString("Use Penalty\n");
		//	break;

		//case ID_STUFF_RESTART:
...
void DeInit()
{
	...
	UnregisterClass("BRIDGEDEMO", g_hInst);		// Free the window class
...
void BuildScene()
{
	PreRenderVisitor PR;
	PR.InitGL();
	BridgeNode::TDParticleContainer Particles;
	Particle::ParticlePtr pP0(new Particle);
	Particle::ParticlePtr pP1(new Particle);
	Particle::ParticlePtr pP2(new Particle);
	Particle::ParticlePtr pP3(new Particle);
	Particle::ParticlePtr pP4(new Particle);
	Particle::ParticlePtr pP5(new Particle);	
	bnu::vector<double> Force(3);
	Force[0] = 0;
	Force[1] = -9.8;
	Force[2] = 0;
	pP0->SetPosition(0,0,0);
	pP1->SetPosition(2.5,-sqrt(3.0)/2.0,0);
	pP2->SetPosition(5, 0, 0);	
	pP3->SetPosition(0,0,-5);
	pP4->SetPosition(2.5,-sqrt(3.0)/2.0,-5);
	pP5->SetPosition(5, 0, -5);	
	
	Particles.push_back(pP0);
	Particles.push_back(pP1);
	Particles.push_back(pP2);
	Particles.push_back(pP3);
	Particles.push_back(pP4);
	Particles.push_back(pP5);
	g_pBridge->Setup(Particles, Force);	
	...
	pDLight->SetAmbient(RGBColor(0.2, 0.2, 0.2));

	//setup material properties
	pMat->SetDiffuse(RGBColor(0.8, 0.2, 0.2));
	pMat->SetAmbient(RGBColor(0.2, 0.2, 0.2));
	pMat->AddChild(g_pBridge);
