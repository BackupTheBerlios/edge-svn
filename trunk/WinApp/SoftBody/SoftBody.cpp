...
Mesh::MeshPtr g_pMesh(new Mesh);
SoftBodyNode::SoftBodyNodePtr g_pSBNode(new SoftBodyNode);
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_SOFTBODY, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, (LPCTSTR)IDC_SOFTBODY);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, (LPCTSTR)IDC_SOFTBODY);
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= (LPCTSTR)IDC_SOFTBODY;
...
void DeInit()
{
	...
	UnregisterClass("SOFTBODY", g_hInst);		// Free the window class
...
void BuildScene()
{
	...
	DirectionLight::DirectionLightPtr pDLight2(new DirectionLight(2));		
	Translate::TranslatePtr pTranslate(new Translate);
	Translate::TranslatePtr pTranslateSB(new Translate);
	Scale::ScalePtr pScaleGround(new Scale);

	//setup light properties
	pDLight->SetDirection(1, -1, 0);
	pDLight1->SetDirection(-1, -1, 0);
	pDLight2->SetDirection(0, -1, -1);
	pDLight->SetDiffuse(RGBColor(0.5, 0.5, 0.5));
	pDLight->SetAmbient(RGBColor(0.01, 0.01, 0.01));
	pDLight1->SetDiffuse(RGBColor(0.5, 0.5, 0.5));
	pDLight1->SetAmbient(RGBColor(0.01, 0.01, 0.01));
	pDLight2->SetDiffuse(RGBColor(0.5, 0.5, 0.5));
	pDLight2->SetAmbient(RGBColor(0.01, 0.01, 0.01));

	//setup material properties
	pMat->SetDiffuse(RGBColor(0.8, 0.0, 0.0));
	pMat->SetAmbient(RGBColor(0.8, 0.0, 0.0));
	...
	g_pMesh->LoadObj("plane.obj");
	pScaleGround->SetScale(10, 0, 10);
	pScaleGround->AddChild(g_pMesh);
	pTranslate->SetTranslate(0,-1,0);
	pTranslateSB->SetTranslate(0,0,0);
	pTranslate->AddChild(pScaleGround);
	//build scene graph	
	pMatGreen->AddChild(pTranslate);
	pTranslateSB->AddChild(g_pSBNode);
	pMat->AddChild(pTranslateSB);	
	pDLight->AddChild(pMat);
	pDLight->AddChild(pMatGreen);
	//pDLight->AddChild(pALight);
	pDLight1->AddChild(pDLight);
	pDLight2->AddChild(pDLight1);
	g_pRoot->AddChild(pDLight2);
}
...
void UpdateScene(float dt)
{
	...
	TS.SetTime(dt);
	std::vector< bnu::vector<double> > Forces;
	//Collide(g_pSBNode, g_pMesh, Forces);	
	//SoftBodyController::SoftBodyControllerPtr pSBC(dynamic_pointer_cast<SoftBodyController>(g_pSBNode->GetDEStateSource()));
	//pSBC->ZeroCollisionForces();
	//pSBC->AddCollisionForces(Forces);
